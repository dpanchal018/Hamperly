...
export default async function AccountOverviewPage() {
  ...
  const totalOutstanding = purchases?.reduce((sum: number, p: any) => sum + Number(p.amount_due), 0) || 0;
...
fs.writeFileSync(path.join(__dirname, 'src', 'app', 'account', 'page.tsx'), accountPage);
